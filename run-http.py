#!/usr/bin/env python3
"""
Odoo CRM MCP Server — SSE transport with per-user credentials via HTTP headers.

Each user passes their Odoo credentials through HTTP headers:
  X-ODOO-URL, X-ODOO-DB, X-ODOO-USERNAME, X-ODOO-PASSWORD, X-ODOO-API-KEY

A raw ASGI middleware injects these into environment variables before each
request reaches FastMCP's SSE handler. This lets multiple users share one
server, each hitting their own Odoo instance.
...
# Disable DNS rebinding protection — this server runs behind nginx which
# forwards the external Host header. The MCP library's default allowed_hosts
# only permits localhost:*, so external domains would get 421.
mcp = FastMCP(
    "odoo-crm",
    host="0.0.0.0",
    port=8766,
...
# ---------------------------------------------------------------------------
# JSON-RPC helpers (per-request, read env vars each call)
# ---------------------------------------------------------------------------
...
def _get_creds() -> dict:
    """Read current request's Odoo credentials from env vars."""
    return {
...
def _authenticate() -> int:
    ...
    if not creds["url"]:
        raise RuntimeError("ODOO_URL not provided — check your credentials")
    ...
    if creds["api_key"]:
        ...
        raise RuntimeError("Either ODOO_API_KEY or ODOO_PASSWORD must be provided")

    if not creds["db"]:
        raise RuntimeError("ODOO_DB not provided")
    if not creds["username"]:
        raise RuntimeError("ODOO_USERNAME not provided")
...
# ---------------------------------------------------------------------------
# Raw ASGI middleware — injects X-ODOO-* headers into env vars per-request.
# Uses raw ASGI (NOT BaseHTTPMiddleware) so SSE streaming works correctly.
# ---------------------------------------------------------------------------
